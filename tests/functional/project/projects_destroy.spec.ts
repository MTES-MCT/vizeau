...
test.group('Projects - Destroy Route', (group) => {
  ...
  test('I can delete one of my projects', async ({ client }) => {
  ...
  test("I can't delete another user's project", async ({ client }) => {
    const user = await UserFactory.with('territoires', 1).create()
    const project = await ProjectFactory.with('user').create()
  ...
  test("I can't delete a project with an invalid id", async ({ client }) => {
    const user = await UserFactory.with('territoires', 1).create()

    const response = await client
      .delete('projets/invalid-project-id')
      .header('Accept', 'application/json')
      .loginAs(user)
      .withCsrfToken()
